 ...
 *
 */
class ProfileadminController extends BackendController
{
	...
	public function registerScript()
	{
		...
		$wgByTypes = UserProfileField::itemAlias('field_type');
	...
	{
		$model = new UserProfileField;
		$scheme = get_class(Yii::app()->db->schema);
		if (isset($_POST['UserProfileField'])) {
			$model->attributes = $_POST['UserProfileField'];

			if ($model->validate()) {
				$sql = 'ALTER TABLE ' . UserProfile::model()->tableName() . ' ADD `' . $model->varname . '` ';
	...
	{
		$model = new UserProfileField('search');
		$model->unsetAttributes(); // clear any default values
		if (isset($_GET['UserProfileField'])) {
			$model->attributes = $_GET['UserProfileField'];
		}
	...
	public function loadModel()
	{
		if ($this->_model === null) {
			if (isset($_GET['id']))
				$this->_model = UserProfileField::model()->findbyPk($_GET['id']);
			if ($this->_model === null)
